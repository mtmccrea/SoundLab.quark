...
Jconvolver {
	var folderPath, compensateLatency, configFileStringArr;
	var <partSize, <maxSize, <numInChannels, <numOutChannels, channelMatrix;
	var <pid;
	...
	*createSimpleConfigFileFromFolder {|partSize = 1024, maxSize = 100000, folderPath, matchFileName = "*.wav", configFilePath, autoConnectToScChannels = 28, autoConnectToSoundcardChannels = 0| //num ins and outs equals number of found kernels, also uses 1:1 matrix, assumes single channel kernels, autoConnect.. if simple number, will connect starting on that channel (with respective number of ins/outs), if array, will map to specified input... maxSize must be larger than numFrames of the kernel
		...
		if(configFilePath.isNil, { //not providing configFilePath creates the file in the same folder as kernels and assumes relative paths to kernels
			...
			configFilePath = folderPath.withTrailingSlash ++ configFileName;
			}, {
				useRelativePaths = false;
		});
		soundFilesArray = SoundFile.collect(folderPath.withTrailingSlash ++ matchFileName);
		...
		//init convolver
		file.write("/convolver/new" + numInChannels.asString + numOutChannels.asString + partSize.asString + maxSize.asString ++ "\n\n");
		...
		soundFilesArray.do({|thisFile, inc|
			var thisPath;
			if(useRelativePaths, {
				thisPath = thisFile.path.asRelativePath(folderPath.withoutTrailingSlash);
				}, {
	...
	*newFromFolder {|folderPath, compensateLatency /*frames*/|
		if(File.exists(folderPath.withTrailingSlash ++ configFileName).not, {
			"no configuration file in the folder, creating one with default config".postln;
			this.createSimpleConfigFileFromFolder(folderPath: folderPath);
		});
		^super.newCopyArgs(folderPath, compensateLatency).init;
	}
	...
	init{
		...
		configFileStringArr = FileReader.read(folderPath.withTrailingSlash ++ configFileName, true);
		configFileStringArr.do({|thisArr, inc|
			...
			if(thisArr[0] == "/convolver/new", {
				...
				partSize = thisArr[3].asInteger;
				maxSize = thisArr[4].asInteger;
			});
		});
		//start convolver
		...
		command = command + folderPath.withTrailingSlash ++ configFileName;
		pid = command.unixCmd;
	}
